...
```{r setup, include = FALSE, echo = FALSE, warning = FALSE, message = FALSE, error = TRUE}
...
```{r}
plot(time, fmu2, type = 'l', lwd = 3, ylim = c(-5,28), xlab = 'Time', ylab = 'Y', col = 6, main = 'Mix of double exponential densities')
...
```{r}
Result <- round(rbind(sase.muf1, rint.muf1, cubint.muf1, ise.muf1), 4)  
Result <- cbind(c('SASE', 'ISE (R base function)','ISE (Cubature package)', 'ISE (MC)'), Result)
...
```{r}
Result <- round(rbind(sase.muf2, rint.muf2, cubint.muf2, ise.muf2), 4)  
Result <- cbind(c('SASE', 'ISE (R base function)','ISE (Cubature package)', 'ISE (MC)'), Result)
colnames(Result) <- c('Method', kername)
Result %>% kable() %>% kable_styling()
```
...
plot(n.vec, resultMat[seq(3, 15, by = 3),3], 
     ...
     ylim = c(0, 1.1), col = 2)
for (i in 4:7) lines(n.vec, resultMat[seq(3, 15, by = 3),i], lwd = 3, col = i - 1)
legend('topright', c('Gaussian', 'Laplace', 'Matern 5/2', 'Matern 3/2', 'PACE'), 
       lty = 1, bty = 'n', col = 2:6, lwd = 3)
```
