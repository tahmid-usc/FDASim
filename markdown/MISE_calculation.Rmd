...
$$
\mu_1(t) = \ N(.2, .05) + \ N(.3, .03) + \ N(.8, .02)    
$$
$$
\mu_2(t) = \ dexp(.2, .05) + \ dexp(.3, .03) + \ dexp(.8, .02)    
$$
...
fdata2 <- fdagen(n = 20, maxt = 10, muf = muf2, theta = c(1, 5, 2))
...
```{r}
plot(time, fmu1, type = 'l', lwd = 3, ylim = c(-5,25), xlab = 'Time', ylab = 'Y', col = 6, main = 'Mix of normal densities')
...
```{r}
plot(time, fmu2, type = 'l', lwd = 3, ylim = c(-5,25), xlab = 'Time', ylab = 'Y', col = 6, main = 'Mix of double exponential densities')
...
plot(n, mse.muf1[,2], col = 1, pch = 16, ylim = c(min(mse.muf1[,-1]), max(mse.muf1[,-1])), type = 'l', 
     xlab = 'Grid size', ylab = 'MSE', lwd = 3, main = 'RASE')
for (i in 3:6) {
